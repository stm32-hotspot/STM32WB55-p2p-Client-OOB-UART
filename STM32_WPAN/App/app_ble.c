...
void APP_BLE_Init(void)
{
  ...
  if(ret == BLE_STATUS_SUCCESS)
  {
    ...
    ret = aci_gap_set_oob_data(0x00, // OOB device type = local device
                               0x00, m_oob_uart_data.address,
                               0x00,
                               0x00,
                               m_oob_uart_data.ra);
    ret |= aci_gap_get_oob_data(1, &m_oob_uart_data.address_type, m_oob_uart_data.address, &m_oob_uart_data.ra_size, m_oob_uart_data.ra);
    ret |= aci_gap_get_oob_data(2, &m_oob_uart_data.address_type, m_oob_uart_data.address, &m_oob_uart_data.ca_size, m_oob_uart_data.ca);
    if(ret != 0)
    {
      HAL_GPIO_WritePin(LED_RED_GPIO_Port, LED_RED_Pin, GPIO_PIN_SET);
...
void APP_BLE_Key_Button3_Action(void)
{
  ...
  APP_DBG_MSG("aci_gap_send_pairing_req, result: 0x%x \n\r", ret);
}
/* USER CODE END FD */
/*************************************************************
 *
...
static void send_oob_data(void)
{
  HAL_UART_Transmit(&hlpuart1, (uint8_t *)&m_oob_uart_data, sizeof(m_oob_uart_data), 1000);
